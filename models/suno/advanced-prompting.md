...
8. **Style box character limit:** Hard ceiling is **1,000 characters** (platform-enforced) — input beyond this is silently truncated mid-phrase, which usually destroys the trailing exclusion block. Target **600–900 characters** (~90–140 words) for production-grade prompts; under 500 characters for electronic/loop genres. Count characters, not words, before submitting. If over, cut redundant exclusions first, then over-specified mic/room detail, then duplicate mood adjectives, then genre-era stacking. Never cut: genre anchor (opening), core instrument list, BPM/groove language, primary exclusion block. See "Hard Limits" section above.
9. **Always include at least a placeholder in the Lyrics box** to prevent style-box bleed.
10. **No artist names in either box.** Never reference a real artist, stage name, or band name in the Style Box or Lyrics Box — including primary descriptions, bracket tags, "in the style of X" phrasing, or seeded lyric lines. Suno v5+ filters or silently degrades prompts containing known names (post-WMG settlement enforcement, November 2025 — see `models/suno/overview.md`). Artist references also collapse to averaged stereotypes rather than specific qualities, and create commercial exposure. Describe the physical sound instead: instrumentation, timbre, era, mix character, delivery style. This rule **overrides** the "Artist Reference Tags" tables in `shared/genre-templates.md` — those are deprecated for output use. See README → Output Contract.
11. **Deliver output as two back-to-back code blocks** — Style Box first, Lyrics Box second, nothing interleaved. The user's workflow is one click to copy each into Suno's two fields. Commentary, mitigation notes, character counts, and caveats go **after** both blocks — never between them, never inside them. See README → Output Contract.
